...
#	batch = mnist.train.next_batch(50)
	if i%100 == 0:
		train_accuracy = accuracy.eval(feed_dict={y_:trainy})
		print "step %d, training accuracy %f"%(i, train_accuracy)
	train_step.run(feed_dict={y_:trainy})
print "???????????????????????????????????????????????????????????????????"
...
print h_pool3_flat
h_fc2=tf.nn.relu(tf.matmul(h_pool3_flat,w_fc1)+b_fc
#y_conv1=tf.nn.softmax(tf.matmul(h_fc2,w_fc2)+b_fc2)
#cross_entropy1 = -tf.reduce_sum(y_*tf.log(y_conv1))
...
#y_conv1.run(feed_dict={y_:testy})
#print "***********************************************************************"
#print "test accuracy %f"%accuracy1.eval(feed_dict={y_:testy})
